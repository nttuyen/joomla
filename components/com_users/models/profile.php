...
class UsersModelProfile extends JModelForm
{
	...
	public function getData()
	{
		if ($this->data === null) {
			...
			/* muinx */
			$query = "SELECT * FROM #__user_profiles WHERE user_id = ".(int) $userId;
			$this->_db->setQuery($query);
			
			$userProfile = $this->_db->loadObjectList();
			
			//build data profile
			foreach($userProfile as $p)
			{
				if($p->profile_key == 'name_of_yours')
					$this->data->name_of_yours = $p->profile_value;
				
				if($p->profile_key == 'website_slogan')
					$this->data->website_slogan = $p->profile_value;
					
				if($p->profile_key == 'date_organization')
					$this->data->date_organization = $p->profile_value;
			}
			/* end */
