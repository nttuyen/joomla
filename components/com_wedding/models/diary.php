...
class weddingModelDiary extends JModel 
{
	var $_data = null;
	...
	function getTotal($user_id)
	{
		if(empty($this->_total))
		{
			...
			$this->_total = generalHelpers::getTotal('#__wedding_diary', 'id', 'user_id='.$user_id);
		}
	}
	...
	{		
		...
		$query = "SELECT * FROM #__wedding_diary WHERE user_id = {$user_id} ORDER BY created_date DESC";
		return $query;
	}
	...
	{
		$db = & JFactory::getDbo();
		$query = 'SELECT * FROM #__wedding_diary WHERE id = '.JRequest::getInt('id').' AND user_id = '.$user_id.' LIMIT 1';
		$db->setQuery($query);
	...
	{
		$db = & JFactory::getDbo();
		$query = 'SELECT * FROM #__wedding_diary WHERE id = '.JRequest::getInt('id').' LIMIT 1';
		$db->setQuery($query);
		return $db->loadObject();
	}
