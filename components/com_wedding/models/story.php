...
class weddingModelStory extends JModel 
{
	...
	function getTotal($user_id)
	{
		if(empty($this->_total))
		{
			...
			$this->_total = generalHelpers::getTotal('#__wedding_stories', 'id', 'user_id='.$user_id);
		}
	}
	...
	{		
		...
		$query = "SELECT * FROM #__wedding_stories WHERE user_id = {$user_id} ORDER BY created_date DESC";
		return $query;
	}
	...
	{
		$db = & JFactory::getDbo();
		$query = 'SELECT * FROM #__wedding_stories WHERE id = '.JRequest::getInt('id').' AND user_id = '.$user_id.' LIMIT 1';
		$db->setQuery($query);
	...
	{
		$db = & JFactory::getDbo();
		$query = 'SELECT * FROM #__wedding_stories WHERE id = '.JRequest::getInt('id').' LIMIT 1';
		$db->setQuery($query);
		return $db->loadObject();
	}
	...
	function save()
	{
		...
		if(isset($_FILES['image']) && $_FILES['image']['error']==0)
		{
			...
			$file = JPATH_ROOT.DS.'images'.DS.'wedding'.DS.'story'.DS;
			if(! JFolder::exists($file))
			{
				JFolder::create($file, 755);
			}
			$time = time();
			$sfile = $file.'story-'.$juser->id.$time.'.jpg';
			
			imagejpeg($p, $sfile, 90);
			imagedestroy($p);
			$data['image'] = 'images/wedding/story/story-'.$juser->id.$time.'.jpg';
		}
