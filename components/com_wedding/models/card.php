...
class weddingModelCard extends JModel 
{
	var $_data = null;
	...
	function getTotal($user_id)
	{
		if(empty($this->_total))
		{
			...
			$this->_total = generalHelpers::getTotal('#__wedding_card', 'id', 'user_id='.$user_id);
		}
	}
	...
	{		
		...
		$query = "SELECT * FROM #__wedding_card WHERE user_id = {$user_id} ORDER BY created_date DESC";
		return $query;
	}
	...
	{
		$db = & JFactory::getDbo();
		$query = 'SELECT * FROM #__wedding_card WHERE id = '.JRequest::getInt('id').' AND user_id = '.$user_id.' LIMIT 1';
		$db->setQuery($query);
	...
	{
		$db = & JFactory::getDbo();
		$query = 'SELECT * FROM #__wedding_card WHERE id = '.JRequest::getInt('id').' LIMIT 1';
		$db->setQuery($query);
		return $db->loadObject();
	}
	...
	{
		$db = & JFactory::getDbo();
		$query = "SELECT intro FROM #__wedding_card_intro WHERE user_id = {$user_id} LIMIT 1";
		$db->setQuery($query);
	...
	function save()
	{
		...
		if(isset($_FILES['image']) && $_FILES['image']['error']==0)
		{
			...
			$file = JPATH_ROOT.DS.'images'.DS.'wedding'.DS.'card'.DS;
			if(! JFolder::exists($file))
			{
				JFolder::create($file, 755);
			}
			$time = time();
			$sfile = $file.'card-'.$juser->id.$time.'.jpg';
			
			imagejpeg($p, $sfile, 90);
			imagedestroy($p);
			$data['image'] = 'images/wedding/card/card-'.$juser->id.$time.'.jpg';
		}
	...
	function saveintro()
	{
		...
		$data['user_id'] = $juser->id;
		$db = & JFactory::getDbo();
		$query = 'SELECT id FROM #__wedding_card_intro WHERE user_id = '.$juser->id.' LIMIT 1';
		$db->setQuery($query);
		$data['id'] = $db->loadResult();
		$data['intro'] = JRequest::getVar('intro', '', '', 'string', JREQUEST_ALLOWHTML);
		
		$row = $this->getTable('cardintro');
		if(!$row->bind($data))
		{
			$this->setError($this->_db->getErrorMsg());
			return false;
