 ...
 * @since		1.5
 */
class JPagination_HpContent extends JObject
{
	/**
	...
	protected function _buildDataObject()
	{
		...
		if ($this->get('pages.current') > 1)
		{
            ...
            $data->start->link	= JRoute::_(preg_replace('/\?start\=[0-9]+/', '', $_SERVER['REQUEST_URI']).'?start=0');
			$data->previous->base	= $page;
			//$data->previous->link	= JRoute::_($params.'&'.$this->prefix.'limitstart='.$page);
            $data->previous->link	= JRoute::_(preg_replace('/\?start\=[0-9]+/', '', $_SERVER['REQUEST_URI']).'?start='.$page);
		}
		...
		if ($this->get('pages.current') < $this->get('pages.total'))
		{
            ...
            $data->next->link	= JRoute::_(preg_replace('/\?start\=[0-9]+/', '', $_SERVER['REQUEST_URI']).'?start='.$next);
			$data->end->base	= $end;
			//$data->end->link	= JRoute::_($params.'&'.$this->prefix.'limitstart='.$end);
            $data->end->link	= JRoute::_(preg_replace('/\?start\=[0-9]+/', '', $_SERVER['REQUEST_URI']).'?start='.$end);
		}
		...
		for ($i = $this->get('pages.start'); $i <= $stop; $i ++)
		{
			...
			if ($i != $this->get('pages.current') || $this->_viewall)
			{
                ...
                $data->pages[$i]->link	= JRoute::_(preg_replace('/\?start\=[0-9]+/', '', $_SERVER['REQUEST_URI']).'?start='.$offset);
			}
		}
		return $data;
	}
