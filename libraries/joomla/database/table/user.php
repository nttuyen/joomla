...
class JTableUser extends JTable
{
	...
	*/
	...
	function store($updateNulls = false)
	{
		...
		$query = "DELETE FROM #__user_profiles WHERE `user_id` = ".(int) $this->id;		
		$this->_db->setQuery($query);
		$this->_db->query();
		...
		//insert into profile
		$profileKeys = array('name_of_yours', 'website_slogan', 'date_organization');
		
		//echo '<pre>'; print_r($post); die;
		
		foreach($profileKeys as $key)
		{
			$query = "INSERT INTO #__user_profiles SET user_id = '".(int) $this->id."', profile_key = '".$key."', profile_value = '".$post['jform'][$key]."'";
			$this->_db->setQuery($query);
			$this->_db->query();
		}
		/* end */
